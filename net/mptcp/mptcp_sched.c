...
static DEFINE_SPINLOCK(mptcp_sched_list_lock);	//Í¬Ê±·ÃÎÊ¹²ÏíÊý¾ÝÊ±µÄËø»úÖÆ£¨ËÀµÈ£©
...
//µ÷ÓÃmptcp_is_def_unavailableºÍmptcp_is_temp_unavailable£¬ÅÐ¶Ï¸ÃsockÊÇ·ñ¿ÉÓÃ
...
//Èç¹û¸ÃskbÒÑ¾­ÔÚskÖÐ£¬²»Ó¦¸ÃÔÙ½«¸Ãskb¼ÓÈësk
...
bool subflow_is_backup(const struct tcp_sock *tp)
{
	...
	// low_prio£ºuse this socket as backup 
	// rcv_low_prio£ºPeer sent low-prio option to us
}
...
* best one
*/
//´Ó·ûºÏÌõ¼þµÄ×ÓÁ÷ÖÐÕÒ³ö×îºÃµÄÒ»Ìõ×ÓÁ÷5
static struct sock
...
struct sock *get_available_subflow(struct sock *meta_sk, struct sk_buff *skb,
	bool zero_wnd_test)
{	//MPTCPÄÚºËÊµÏÖµÄÈý¸ö×é³É²¿·ÖÖ®Ò»£ºMulti-path control bock(mpcb) 
...
* receive-buffer.
*/
//·µ»ØÏÂÒ»¸öÒª´«ÊäµÄÊý¾Ý¶Î£¬ÓÅÏÈÑ¡Ôñreinject¶ÓÁÐÄÚµÄÊý¾Ý¶Î
